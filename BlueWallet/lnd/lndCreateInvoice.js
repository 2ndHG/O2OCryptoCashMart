...
const LNDCreateInvoice = () => {
  ...
  const { walletID, uri, creatingInvoice, order } = useRoute().params;
  const wallet = useRef(
  ...
  useEffect(() => {
    console.log('轉跳到建立收錢的支票的頁面');
    //console.log(order)
    if(creatingInvoice) {
      //console.log("收錢: ", order)
      setAmount(order.price);
      setDescription('此條碼用於付款給攤商 '+order.price+'元')
    }
  ...
  //按下Create Invoice 後執行這個函式來建立支票
  const createInvoice = async () => {
    setIsLoading(true);
    try {
      let invoiceAmount = amount;
      switch (unit) {
        //根據金額欄的幣種轉換成sat，存入invoiceAmount變數中
        case BitcoinUnit.SATS:            //本身就是sat
          invoiceAmount = parseInt(invoiceAmount); // basically nop
          break;
        case BitcoinUnit.BTC:             //比特幣
          invoiceAmount = currency.btcToSatoshi(invoiceAmount);
          break;
        case BitcoinUnit.LOCAL_CURRENCY:  //美金
      ...
      <View style={[styles.root, styleHooks.root]}>
        ...
        <View style={[styles.amount, styleHooks.amount]}>
          <KeyboardAvoidingView enabled={!Platform.isPad} behavior="position">
            ...
            />
            {creatingInvoice? <View style={[styles.fiat, styleHooks.fiat]}>
              <TextInput
                //支票的description欄位
              ...
              {lnurlParams ? null : renderScanClickable()/*這是掃描按鈕*/}
            </View>:<></>}
