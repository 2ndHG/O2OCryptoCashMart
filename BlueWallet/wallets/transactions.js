...
const WalletTransactions = () => {
  ...
  //happy
  const { payingInvoice, creatingInvoice, order } = useRoute().params;
  ...
  useEffect(() => {
    ...
    console.log('creatingInvoice', creatingInvoice)
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    return (
      <View style={styles.flex}>
        ...
        <View style={{flex: 1, alignItems:'center'}}>
          {
            creatingInvoice?
            <Text style={{fontSize:24}}> 請按下面的Recieve!</Text>:
            <></>
          }
          {
          (payingInvoice)&&<Text style={{fontSize:24}}>請按下面的Send!</Text>
          }
        </View>
  ...
  //這個函式在錢包頁面按下send後執行，會分別以閃電錢包跟一般錢包做區隔
  const sendButtonPress = () => {
    if (wallet.chain === Chain.OFFCHAIN) {
      console.log('在閃電錢包頁面按下按鈕(send)');
      return navigate('ScanLndInvoiceRoot', { screen: 'ScanLndInvoice', params: { walletID: wallet.getID(), payingInvoice: payingInvoice, order: order } });
    }
      ...
      <FContainer ref={walletActionButtonsRef}>
        {wallet.allowReceive() && (
          <FButton
            ...
            onPress={() => {
              if (wallet.chain === Chain.OFFCHAIN) {
                navigate('LNDCreateInvoiceRoot', { screen: 'LNDCreateInvoice', params: { walletID: wallet.getID(), creatingInvoice: creatingInvoice, order: order } });
              } else {
