...
//這個元件是創建支票後跳出來的QRcode、資訊跟share
const LNDViewInvoice = () => {
  const { invoice, walletID, isModal, order } = useRoute().params;
  ...
  useEffect(() => {
    ...
    //因為一開始paymet_request沒有 所以等三秒再發送
    // setTimeout(() => {
    //   createInvoice(invoice.payment_request);
    // }, 10000); 
  ...
  //happy
  const createInvoice = async (payment_request) => {
      ...
      return (
        <ScrollView>
          <View style={[styles.activeRoot, stylesHook.root]}>
            ...
            <Text style={{fontSize: 24}}>付款支票已上傳至伺服器</Text>
            <BlueSpacing20 />
            <BlueButton
