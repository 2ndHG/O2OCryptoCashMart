...
import { BlueButton, BlueCard, BlueDismissKeyboardInputAccessory, BlueLoading, BlueSpacing40, SafeBlueArea } from '../../BlueComponents';
...
const ScanLndInvoice = () => {
  ...
  const { walletID, uri, invoice, payingInvoice, order } = useRoute().params;
  ...
  //happy
  const insertLndAddress = () => {
    setDestination(order.訂單付款lnd)
  }
  useEffect(() => {
    
    console.log('scanLndInvoice useEffect');
    console.log('轉跳到閃電錢包傳送交易頁面');
    //設定付款位置
    if(payingInvoice)
      insertLndAddress();
    //console.log(order);
  ...
  //通知付款
  const completePayingInvoice = async () => {
    console.log('向伺服器通知付款成功...');
    console.log(order)
    let 訂單IPFSHash = "";
    await ipfs.add(order.toIPFS, (err, ifpsResults) => {
        if (err) 
        {
            console.log(err);
            return ;
        ...
        訂單IPFSHash = ifpsResults;
        try {
          fetch(`http://10.0.2.2:3101/order/updateOrder`, {
              headers: { "Content-Type": "application/json" },
              method: 'POST',
              body: JSON.stringify({
                  command: '付款成功',
                  訂單GUID: order.訂單GUID,
                  訂單IPFSHash: 訂單IPFSHash
              }) 
          }).then((response) => {
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <KeyboardAvoidingView enabled behavior="position" keyboardVerticalOffset={20}>
            ...
            <BlueCard>
              ...
              <BlueSpacing40/>
              {(payingInvoice)&&<Text style={{fontSize:24}}>    按pay付款!</Text>}
            </BlueCard>
          </KeyboardAvoidingView>
